  ...
  return (
    <header className="z-[999] relative">
      ...
      <nav
        ...
        sm:top-[2.3rem] sm:h-[initial] sm:py-0"
      >
        <ul
          className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 
          text-[0.9 rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5"
        >
          {links.map((link) => (
            <motion.li
            ...
            >
              <Link
                className="flex w-full items-center justify-center px-1.5 
                hover:text-gray-950 font-inter font-extrabold transition"
                href={link.hash}
              >
                {link.name}
              </Link>
